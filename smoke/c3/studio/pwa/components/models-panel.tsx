  ...
  return (
    <section className="grid flex-1 gap-4 lg:grid-cols-[minmax(0,1fr)_360px]">
        ...
        <CardContent className="grid gap-4 p-4 sm:grid-cols-2 sm:p-6">
          {MODEL_CATALOG.map((model) => (
            <article key={model.id} className={`relative overflow-hidden rounded-[22px] border p-4 sm:p-5 ${cache[model.id] ? 'border-emerald-300/18 bg-emerald-300/[0.035]' : 'border-white/8 bg-white/[0.025]'}`}>
              ...
              <p className="mt-3 text-xs leading-5 text-slate-400">{model.verifiedUse}</p>
              <dl className="mt-5 grid grid-cols-2 gap-2 text-xs">
                ...
                <div className="rounded-xl bg-black/10 p-3"><dt className="text-muted-foreground">Test mAP50–95</dt><dd className="mt-1 font-mono">{(model.map5095 * 100).toFixed(1)}%</dd></div>
                <div className="rounded-xl bg-black/10 p-3"><dt className="text-muted-foreground">Test mAP50</dt><dd className="mt-1 font-mono">{(model.map50 * 100).toFixed(1)}%</dd></div>
                ...
                <div className="col-span-2 rounded-xl bg-black/10 p-3"><dt className="text-muted-foreground">Experiment protocol</dt><dd className="mt-1 leading-5">{model.protocol}</dd></div>
              </dl>
              {activeId === model.id && (
      ...
      <aside className="flex flex-col gap-3">
        ...
        <Card className="border-white/10 bg-card/60 ring-0"><CardContent className="p-4"><div className="mb-3 flex items-center gap-2 font-medium"><ShieldCheck className="size-4 text-cyan-300" /> Research boundary</div><p className="text-xs leading-5 text-muted-foreground">The app ships only the two canonical P1 V-PEFT exports. Reported test metrics belong to their fixed dataset splits; phone-camera results are not an extension of those metrics.</p></CardContent></Card>
