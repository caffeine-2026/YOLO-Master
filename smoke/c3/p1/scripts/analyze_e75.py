#!/usr/bin/env python3
"""Analyze the immutable C3 P1 seed824 75-epoch comparison runs."""
...
def normalized_curve(path: Path) -> list[dict[str, float]]:
    ...
    if len(rows) != 75 or [row["epoch"] for row in rows] != list(range(1, 76)):
        raise ValueError(f"Expected epochs 1-75 in {path.relative_to(REPO_ROOT)}")
    return rows
...
def load_runs() -> dict[str, dict[str, object]]:
    ...
    for dataset, dataset_name, _ in DATASETS:
        for method, method_name, _ in METHODS:
            identifier = run_id(dataset, method, 75)
...
def comparison_rows(runs: dict[str, dict[str, object]]) -> list[dict[str, object]]:
    rows = []
    for dataset, dataset_name, _ in DATASETS:
        full = runs[run_id(dataset, "full_sft", 75)]["metrics"]
        for method, method_name, _ in METHODS:
            metrics = runs[run_id(dataset, method, 75)]["metrics"]
...
def plot_curves(runs: dict[str, dict[str, object]]) -> list[str]:
    output_dir = P1_ROOT / "visualizations" / "convergence_e75"
    ...
    for dataset, dataset_name, prefix in DATASETS:
        for key, ylabel, suffix in definitions:
            ...
            for method, method_name, _ in METHODS:
                curve = runs[run_id(dataset, method, 75)]["curve"]
                axis.plot([row["epoch"] for row in curve], [row[key] for row in curve], label=method_name, linewidth=2)
            axis.set(title=f"{dataset_name}: {ylabel} vs epoch (75-epoch candidate)", xlabel="Epoch", ylabel=ylabel)
...
def convergence_rows(runs: dict[str, dict[str, object]]) -> list[dict[str, object]]:
    ...
    for run in runs.values():
        ...
        best = max(range(75), key=values.__getitem__)
        rows.append(
            {
...
def fairness_audit(runs: dict[str, dict[str, object]]) -> dict[str, object]:
    ...
    for dataset, _, _ in DATASETS:
        dataset_runs = [runs[run_id(dataset, method, 75)] for method, _, _ in METHODS]
        ...
        for method, _, _ in METHODS:
            current = runs[run_id(dataset, method, 75)]["resolved"]
            previous = yaml.safe_load(
                (P1_ROOT / "logs" / run_id(dataset, method, 50) / "resolved_config.yaml").read_text(encoding="utf-8")
            )
            old_parity[run_id(dataset, method, 75)] = {
                key: current.get(key) == previous.get(key) for key in COMMON_KEYS
            }
        full = runs[run_id(dataset, "full_sft", 75)]
        frozen = runs[run_id(dataset, "frozen_backbone", 75)]
        vpeft = runs[run_id(dataset, "vpeft", 75)]
    ...
    checks = {
        ...
        "epochs_75_for_all": all(run["resolved"]["epochs"] == 75 for run in runs.values()),
        "all_common_fields_equal": all(common_checks.values()),
        "only_epoch_changed_from_e50": all(all(values.values()) for values in old_parity.values()),
    ...
    return {
        ...
        "restart_reason": "The 50-epoch cosine schedule is not semantically equivalent to a from-scratch 75-epoch schedule.",
        "reference": {key: reference.get(key) for key in COMMON_KEYS},
        "common_fields": common,
        "common_checks": common_checks,
        "e50_parity_checks_excluding_epochs": old_parity,
...
def e30_e50_e75_rows(runs: dict[str, dict[str, object]], convergence: list[dict[str, object]]) -> tuple[list[dict[str, object]], bool]:
    ...
    for dataset, dataset_name, _ in DATASETS:
        for method, method_name, _ in METHODS:
            ...
            new = runs[run_id(dataset, method, 75)]["metrics"]
            rows.append(
                {
                    "dataset": dataset_name,
                    "method": method_name,
                    "map50_95_e30_test": old["test"]["map50_95"],
                    "map50_95_e50_test": middle["test"]["map50_95"],
                    "map50_95_e75_test": new["test"]["map50_95"],
                    "delta_e30_e50": middle["test"]["map50_95"] - old["test"]["map50_95"],
                    "delta_e50_e75": new["test"]["map50_95"] - middle["test"]["map50_95"],
                    "best_validation_epoch_e75": convergence_by_id[new["run_id"]]["best_epoch"],
                }
            )
    rankings = []
    for dataset, _, _ in DATASETS:
        stage_orders = []
        for epoch in (30, 50, 75):
            stage_orders.append(
                tuple(
                    method
                    for method, _, _ in sorted(
                        METHODS,
        ...
        rankings.append(len(set(stage_orders)) == 1)
    return rows, all(rankings)
...
def tradeoff_rows(runs: dict[str, dict[str, object]]) -> list[dict[str, object]]:
    ...
    for dataset, dataset_name, _ in DATASETS:
        full = runs[run_id(dataset, "full_sft", 75)]["metrics"]
        vpeft = runs[run_id(dataset, "vpeft", 75)]["metrics"]
...
def write_docs(
    ...
    final_state = (
        "75 epoch 已冻结为 multi-seed 的统一最终预算。"
        if decision == "KEEP_75"
        else "75 epoch 仍是 convergence candidate，不能称为 final P1 结果；下一阶段须六种条件统一扩展到 100 epoch。"
    )
    gate_state = "YES" if multiseed_ready else "NO"
    ranking_text = "稳定" if ranking_stable else "未稳定"
    audit = f"""# C3 P1 75-Epoch 收敛审计
...
`{decision}`。{plateau_count}/6 组达到 plateau 判据，{not_converged_count}/6 组仍超过固定阈值。{final_state}

## 6. Fairness

`Protocol fairness: {fairness['status']}`。除 epoch 50→75 外，模型、预训练权重、split、100-image 预算、batch、imgsz、optimizer、lr、weight decay、scheduler、augmentation、seed、freeze 边界与 V-PEFT 配置均保持一致。

## 7. 30/50/75

30/50/75 三个预算下的锁定 test mAP50-95 方法排序为“{ranking_text}”。这只说明当前 seed 的排序状态，不构成 multi-seed 方法结论。详见 `../results/e30_e50_e75.csv`。

## 8. Multi-seed gate

`MULTISEED_READY = {gate_state}`。本轮未运行 seed825/826。
"""
    (P1_ROOT / "docs" / "CONVERGENCE_AUDIT_E75.md").write_text(audit, encoding="utf-8")
    ...
    report_stage = "Final-Epoch Protocol" if decision == "KEEP_75" else "Convergence Candidate"
    report = f"""# C3 P1 对照报告（seed824，75-Epoch {report_stage}）
...
见 `../results/e30_e50_e75.csv`。三个预算下的方法排序为“{ranking_text}”；单 seed 排序不能外推为总体方法优劣。
...
def main() -> int:
    ...
    e30_e50_e75, ranking_stable = e30_e50_e75_rows(runs, convergence)
    tradeoff = tradeoff_rows(runs)
    plateau_count = sum(row["status"] == "CONVERGED_OR_PLATEAU" for row in convergence)
    decision = "KEEP_75" if plateau_count >= 5 and ranking_stable else "EXTEND_ALL_TO_100"
    multiseed_ready = decision == "KEEP_75" and fairness["status"] == "PASS"

    write_csv(P1_ROOT / "results" / "pilot_seed824_e75.csv", comparison)
    (P1_ROOT / "results" / "pilot_seed824_e75.md").write_text(
        "# C3 P1 seed824 75-Epoch Candidate\n\n30 epoch 结果仅为历史 pilot；本表为当前 75 epoch convergence candidate。\n\n"
        + markdown_table(comparison)
        + "\n",
        encoding="utf-8",
    )
    write_csv(P1_ROOT / "results" / "convergence_epochwise_e75.csv", epochwise_rows(runs))
    write_csv(P1_ROOT / "results" / "e30_e50_e75.csv", e30_e50_e75)
    write_csv(P1_ROOT / "results" / "tradeoff_e75.csv", tradeoff)
    json_write(P1_ROOT / "evidence" / "protocol_fairness_audit_e75.json", fairness)
    json_write(
        P1_ROOT / "evidence" / "convergence_audit_e75.json",
        {
            "schema_version": 1,
            "metric": "mAP50-95",
            "rule": {
                "previous_window": "epochs 66-70",
                "last_window": "epochs 71-75",
            ...
            "ranking_stable_across_e30_e50_e75": ranking_stable,
            "execution_policy": "restart_all",
            "plots": plots,
        },
    )
    write_docs(comparison, convergence, fairness, tradeoff, decision, plots, ranking_stable, multiseed_ready)
    print(
        json.dumps(
            {
                "runs": len(runs),
