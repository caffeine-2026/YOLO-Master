...
결과: 모두 PASS. `fallback.py`의 import ordering, module alias, future annotation, `__all__` ordering 네 lint 범주는 이번 동작 수정 전부터 존재한 파일 수준 부채라 명시적으로 제외했다. JSON/YAML 948개 파싱, Markdown local link 52개, publishable file 1,465개 privacy 검사와 14개 그래프 provenance 검사는 통합 validator에서 PASS했다.
